...
LineSensors lines(line_falling);  //line object that checks lines
const bool enable_lines = true; 	//determines if lines are disabled
void dummycheck()		//dummy function for line callback
{}  			
...
public:
	...
	void checkLines()  //updates status of lines and stores it
	{
		lines.CheckLines();
	}

	bool leftFront()//returns status of lines
	{
		return lines.GetLineDetected(1);
	}

	bool rightFront()
	{
		return lines.GetLineDetected(2);
	}

	bool leftBack()
	{
		return lines.GetLineDetected(3);
	}

	bool rightBack()
	{
		return lines.GetLineDetected(4);
	}
...
private:
	...
	void initialize()
	{
		...
		lines.SetFunctionPtr(dummycheck);
