...
{
	// use a 1/256 clock divider
	USHORT clockDivider = 256;
...
void Timer::internalStart(UBYTE timer)
{
	switch (timer)
	{
		...
		case 1:
		{
			// clear the relevant register bits
			TCCR1A &= ~(_BV(COM1A1) | _BV(COM1A0));
			TCCR1A &= ~(_BV(WGM11) | _BV(WGM10));
			TCCR1B &= ~(_BV(WGM12) | _BV(WGM13));
			//TCCR1A |= _BV(COM1A1);

			//set to fast PWM mode with OCR1A as the top and update OCR1A at bottom
			//this will prevent the glitch of odd intervals when updating OCR1A
			TCCR1A |= _BV(WGM11) | _BV(WGM10);
			TCCR1B |= _BV(WGM12) | _BV(WGM13);
			TIMSK1 |= _BV(OCIE1A);
			break;
