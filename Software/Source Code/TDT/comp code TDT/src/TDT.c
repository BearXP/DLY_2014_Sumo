...
const USHORT IR_push_threshold_on = 300;
const USHORT IR_push_threshold_off = 300;
...
const USHORT search_speed = 400;
const USHORT push_speed =150;
	...
	while (true)
	{
		...
		switch (state)
		{
			...
			case state_obstacle_front:
			{
				...
				if(fl<IR_push_threshold_on||fr<IR_push_threshold_on) //obstacle in not front
				{
					forward(push_speed,push_speed + (rmotor_diff/2));
				}
				else
				{
